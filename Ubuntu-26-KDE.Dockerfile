...
RUN apt-get update && \
    ...
    if [ "$BUILD_KDE" = "conc" ]; then \
        ...
        polkit-kde-agent-1 libpam-systemd libpam-modules libpam-kwallet plasma-session-x11 language-pack-kde-zh-hans language-pack-zh-hans qt6-translations-l10n; \
    fi && \
    ######################################################################################################
    #输入法 fcitx5 (可选)
